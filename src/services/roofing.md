---
layout: layouts/service.njk
title: "Roofing"
seoTitle: "Pole Barn & Metal Roofing in Michigan"
subtitle: "Roof installation, replacement, and repair"
description: "Roof installation, replacement, and repair for pole barns, garages, and homes across southeast Michigan. Licensed, insured. Call (248) 625-2334."
category: "Repair & Maintenance"
image: "/images/projects/pole-barn-roof-deck-framing.jpg"
order: 7
features:
  - "Metal and asphalt shingle roofing options"
  - "Standing seam and screw-down metal panel systems"
  - "Re-roofing over existing materials when possible"
  - "Ridge caps, trim, and gutter systems"
gallery:
  - src: "/images/projects/pole-barn-roof-deck-framing.jpg"
    alt: "Pole barn roof deck under construction with radiant barrier sheathing, blue sky"
    caption: "A pole barn roof deck under construction"
faq:
  - q: "Should I choose metal or shingle roofing?"
    a: "It depends on the building and your budget. Metal roofing costs more upfront but holds up well against Michigan snow, wind, and ice over the long run, while asphalt shingles are a more budget-friendly option for many homes and outbuildings. We will go over both options honestly during your free estimate."
  - q: "Can you put a metal roof over an existing shingle roof?"
    a: "In many cases, yes. Re-roofing over existing shingles can save on tear-off costs. We check the existing roof deck and structure first to confirm it can support the new metal panels, and add furring strips where needed for proper ventilation."
  - q: "What if my roof just needs a repair, not a full replacement?"
    a: "Not every roofing problem requires a full tear-off. We repair leaks, storm damage, and isolated wear on both metal and shingle roofs, and will tell you honestly when a repair makes more sense than a full replacement."
tags:
  - "roofing"
  - "metal roofing"
  - "pole barns"
  - "michigan"
---

## Roof Installation, Replacement, and Repair

CNO Pole Barns installs, replaces, and repairs roofs on pole barns, garages, horse barns, and homes across Oakland, Genesee, Lapeer, and Macomb counties. We work in both metal and asphalt shingle roofing. New construction or a full replacement on a roof that's reached the end of its life, we'll walk you through the options and give straightforward pricing before any work starts.

## Roofing That Can Take Michigan Weather

Snow load, ice, and wind-driven rain are hard on any roof. We install proper ridge ventilation, drip edge, and flashing details so water and ice do not find a way in, and we size the roofing system to the building rather than using a one-size-fits-all approach. CNO Pole Barns is a Certified Federal Steel Builder, and our crews install roofing on new construction as well as replacement and repair jobs on buildings we did not build.

## Free Estimates on Every Roofing Job

CNO Pole Barns was established in 1970, and today the business is owned and run by Zosim Serban. We are licensed and insured, hold an A+ rating with the Better Business Bureau, and give a free estimate for every roofing job, whether it is new construction, a full replacement, or a repair. Call (248) 625-2334 to schedule a time for us to look at your roof.
